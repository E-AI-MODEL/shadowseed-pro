# External research papers

This directory contains external source-reference material used during Shadowseed research and review. It is not part of the Shadowseed manuscript itself.

- `conflicting_evidence_fact_checking_arxiv_2505_17762.pdf`: *Resolving Conflicting Evidence in Automated Fact-Checking: A Study on Retrieval-Augmented LLMs* (Ge et al., arXiv:2505.17762).

Repository boundary:

- `paper/` contains the Shadowseed manuscript source, bibliography, and compiled publication artifact.
- `data/papers/` contains external papers and supporting source-reference material.
- External source PDFs should not be moved under `paper/data/`, because that would mix publication artifacts with research inputs.

The original extensionless source file is retained under `archive/source-data/`.
