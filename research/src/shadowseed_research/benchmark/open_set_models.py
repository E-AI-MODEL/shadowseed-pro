...
The packaged `shadowseed:data/open_set_models.json` registry is the single source
...
DEFAULT_REGISTRY = "shadowseed:data/open_set_models.json"


def _registry_text(registry_path: str) -> str:
    if registry_path == DEFAULT_REGISTRY:
        return (
            resources.files("shadowseed")
            .joinpath("data", "open_set_models.json")
            .read_text(encoding="utf-8")
        )
    return Path(registry_path).read_text(encoding="utf-8")


def load_models(registry_path: str = DEFAULT_REGISTRY) -> list[dict[str, Any]]:
    data = json.loads(_registry_text(registry_path))
