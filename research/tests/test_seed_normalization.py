...
from shadowseed_research.benchmark.open_set_seed_review import detect_embedding
...
def test_expand_short_fragments_disabled_keeps_model_fragments_as_is():
    # For real language-model output, short stubs stay visible instead of being
    # dressed up with the historical "ontbreekt" suffix.
    normalized = normalize_detection_candidates(
...
def test_manager_ingest_passes_expand_flag_through():
    ...
    # Without the flag, the same input would produce "TORONTO ontbreekt."
...
def test_split_broad_disabled_keeps_comma_sentence_whole():
    # Model output containing a natural comma must stay one seed instead of
    # being split into unrelated fragments.
...
def test_ingest_model_mode_keeps_unique_seed_ids_for_paraphrases():
    # A degenerate embedding makes every candidate look identical. With semantic
    # deduplication enabled, distinct gaps would collapse onto one seed id. Model
    # mode disables that path so exact duplicate handling remains explicit.
