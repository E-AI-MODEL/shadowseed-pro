...
The final production-local candidate then runs `.github/workflows/production-release-assurance.yml` manually against the published tag. That workflow checks that the tag still resolves to unchanged current `main`, redownloads and verifies the published checksum set, validates `PROVENANCE.json` and all three standalone manifests, generates a Sigstore-backed GitHub artifact attestation for every subject listed in `SHA256SUMS`, verifies every subject with `gh attestation verify`, and checks `main` again after assurance. The resulting workflow run ID is mandatory final release evidence.
...
- Sigstore-backed GitHub artifact attestation generated by the pinned `actions/attest` release-assurance step for every checksum-listed release subject, followed by successful `gh attestation verify` policy verification.
