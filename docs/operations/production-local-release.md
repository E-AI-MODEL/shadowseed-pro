...
The artifact attestation is the declared cryptographic provenance path for the distributed production-local release subjects. It proves repository/workflow provenance and subject digests; it does not establish that the code is vulnerability-free or grant extra seed authority.

The macOS standalone is intentionally **ad-hoc signed** and does not require Apple Developer ID certificates, App Store Connect keys or notarization credentials. CI still requires the final app bundle seal to pass strict `codesign --verify`, then archives the exact user-facing bundle, extracts that archive again, verifies the seal again and executes the frozen self-test from the round-tripped app.

A browser download may add the `com.apple.quarantine` attribute. Because this release is not Apple-notarized, macOS may block a normal first double-click. The macOS ZIP therefore contains:

- `Shadowseed.app`;
- `Open Shadowseed.command`;
- `README_FIRST_START.txt`.

The first-launch helper removes `com.apple.quarantine` recursively **only from the adjacent `Shadowseed.app` bundle** and then opens that app. It does not disable Gatekeeper globally, does not use `sudo`, and does not change system-wide macOS security settings. If Finder also blocks the downloaded helper, the user can Control-click the helper, choose **Open**, and confirm once. Later launches can use `Shadowseed.app` directly.

The standalone manifest must report `macos_signature_mode=adhoc`, `macos_notarized=false`, `macos_gatekeeper_assessed=false`, `macos_first_launch_helper=true`, and a successful archive round-trip. `Release Workbench` rejects any macOS artifact that claims stronger Apple platform verification than was actually performed.

Apple Developer ID signing and notarization are **not claimed** for this distribution. Windows Authenticode signing is also not currently claimed. These platform-signing boundaries do not make checksum, provenance, SBOM, artifact-attestation or frozen self-test verification optional.
