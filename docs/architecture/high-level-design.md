...
The reusable integration boundary is `ShadowseedEngine`: a model-independent two-phase API that prepares bounded context, lets the host call its own model, and observes the visible answer afterwards. The Workbench is the reference product surface and currently uses the same canonical `ShadowChatSession` runtime directly rather than calling the `ShadowseedEngine` facade.
...
| **Advanced / research** | Built-in access to stored blinded comparisons and advanced session inspection; separate benchmark/evaluation suites are installed through the research distribution |
...
|---|---|---|
| Status | `NEW` | Newly recorded and available for investigation, without authority |
...
A stored comparison can be loaded as a blinded A/B pair in the built-in **Advanced / research** tab. This product comparison path does not require the separate research distribution. The separate distribution is for benchmark and evaluation suites beyond the ordinary Workbench.
...
|---|---|---|
| Full report | Detailed session review and qualitative research | Prompts, visible answers, seeds, Gate and influence records, free-text feedback, and integrity metadata |
...
Shadowseed Pro exposes a reusable engine integration boundary alongside five product capabilities:

| Product part | What it gives the user |
|---|---|
| Model-independent engine | A reusable `prepare_turn` / `observe_turn` boundary that never owns the host model call |
...
A fresh tester can download the application, select a model, start chatting, inspect what Shadowseed noticed, understand why a seed did or did not influence an answer, compare the same message with SSL off, and export a verifiable record. The Workbench reaches that behavior through the canonical chat/runtime path. A host developer can integrate the same live pipeline through `ShadowseedEngine` without adopting the Workbench or handing model credentials to Shadowseed.
...
- [`engine/README.md`](../engine/README.md): the standalone model-independent engine contract.
- [`repository-structure.md`](repository-structure.md): the product/research package boundary and compatibility rules.
...
Use the linked canonical documents and runtime tests for exact behavior. Update the runtime source anchor whenever product or architecture behavior changes.
