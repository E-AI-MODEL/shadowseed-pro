...
"Non-bypassable" is scoped to the supported runtime and public API: no normal caller, helper, probe, SSOT component, or integration is given an authority mutator outside the Gate, and a static test over `src/` enforces that runtime code never reaches for one. It is **not** a claim that arbitrary in-process Python cannot mutate a seed — explicit, clearly named `unsafe_set_authority` / `unsafe_install_seed` hooks exist for tests and benchmarks, and nothing stops third-party code from calling them. The guarantee is about the public API surface, not the Python object model. See [Assurance boundaries](#assurance-boundaries) below.
...
| Influence requires one atomic point-of-use decision enforcing specific checks (weight > 0, promoted, live current-version Gate-event link, no blocking contradiction) | [`AgentSafetyContract.decide_and_record`](src/shadowseed_agent/agent_contract.py) | [`test_point_of_use.py`](tests/test_point_of_use.py) |
...
- **The point-of-use contract enforces specific checks, not universal safety.** [`AgentSafetyContract.decide_and_record`](src/shadowseed_agent/agent_contract.py) allows influence only when the seed has `weight > 0`, is promoted, links to a live Gate event of the seed's *current* `authority_version` (stale authorizations are rejected), and carries no blocking contradiction; every decision — allowed or denied — is recorded ([`test_point_of_use.py`](tests/test_point_of_use.py), [`test_agent_safety_contract.py`](tests/test_agent_safety_contract.py)). "Zero-trust at the agent boundary" names this specific gate; it does **not** imply complete policy enforcement, protection against every integration that ignores the contract, or safety against all prompt-injection or evidence-poisoning attacks (see [Not established](#not-established)).
